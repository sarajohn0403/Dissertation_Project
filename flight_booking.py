...
# Define the FSM model for the flight booking scenario
def create_fsm_model():
    # Define states
    initial_state = State("Initial")
    flight_selected_state = State("Flight Selected")
    passenger_info_state = State("Passenger Information")
    payment_state = State("Payment")
    booking_confirmed_state = State("Booking Confirmed")

    # Define transitions
    transitions = [
        Transition("Select Flight", initial_state, flight_selected_state),
        Transition("Enter Passenger Info", flight_selected_state, passenger_info_state),
        Transition("Make Payment", passenger_info_state, payment_state),
        Transition("Confirm Booking", payment_state, booking_confirmed_state)
    ]

    # Create FSM model
    finite_model = {
        "states": [initial_state, flight_selected_state, passenger_info_state, payment_state, booking_confirmed_state],
        "transitions": transitions
    }
    return finite_model

# Define the SMM model for the flight booking scenario
...
# Create the FSM model for the flight booking scenario
fsm_model = create_fsm_model()

# Create the SMM model for the flight booking scenario
