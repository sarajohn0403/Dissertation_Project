...
# Define the FSM model for the login form scenario
def create_fsm_model():
    # Define states
    initial_state = State("Initial")
    username_entered_state = State("Username Entered")
    password_entered_state = State("Password Entered")
    login_success_state = State("Login Success")
    login_failure_state = State("Login Failure")

    # Define transitions
    transitions = [
        Transition("Submit", initial_state, username_entered_state),
        Transition("Invalid Credentials", username_entered_state, login_failure_state),
        Transition("Valid Credentials", username_entered_state, password_entered_state),
        Transition("Submit", password_entered_state, login_success_state),
        Transition("Invalid Credentials", password_entered_state, login_failure_state)
    ]

    # Create FSM model
    finite_model = {
        "states": [initial_state, username_entered_state, password_entered_state, login_success_state,
                   login_failure_state],
        "transitions": transitions
    }
    return finite_model

# Define the SMM model for the login form scenario
...
# Create the FSM model for the login form scenario
fsm_model = create_fsm_model()

# Create the SMM model for the login form scenario
...
print("FSM Model - Quantitative Metrics: ")
display_metrics(*fsm_metrics)
print()

# Display metrics for the SMM model
print("SMM Model - Quantitative Metrics: ")
